			...
			for {
				...
				case <-ticker.C:
					...
					if len(dataPublisher.drs.items) > 0 {
						log.Println("[RAVENPOD] No. of records in data pipeline", len(dataPublisher.drs.items))
						...
						log.Println("[RAVENPOD] No records in data pipeline")
					}
				case <-quit:
					ticker.Stop()
					return
...
func (dp *DataPublisher) PushRecord(record interface{}, partitionKey string) {
	...
	if err != nil {
		log.Println("[RAVENPOD] Error marshalling the record data", err)
		return
	}
...
func (dp *DataPublisher) flushRecordSet(streamName string, items *[]*kinesis.PutRecordsRequestEntry) {
	...
	if err != nil {
		log.Fatal("[RAVENPOD] Error pushing recordset to data pipeline", err)
	} else {
		log.Println("[RAVENPOD] Recordset pushed to data pipeline.", streamName)
		*items = nil
	}
}
